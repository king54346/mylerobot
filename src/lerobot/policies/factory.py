"""
策略工厂模块 (Policy Factory Module)
==================================

本模块提供了创建和配置策略模型的工厂函数。

主要功能：
- get_policy_class(): 根据名称加载策略类（动态导入）
- make_policy_config(): 创建策略配置对象
- make_policy(): 创建并配置策略模型
- make_pre_post_processors(): 创建预/后处理器
...
- Pi0/Pi05: Physical Intelligence 策略
- SmolVLA: 小型视觉语言动作模型
- SAC: Soft Actor-Critic 强化学习
- Gr00t: NVIDIA Gr00t 模型
- XVLA: 跨模态视觉语言动作模型
- WallX: Qwen2.5-VL 基础模型
"""
...
def get_policy_class(name: str) -> type[PreTrainedPolicy]:
    ...
    此函数使用动态导入，避免一次性加载所有策略类到内存，
    提高启动速度并减少依赖。

    参数:
        name: 策略名称。支持的名称有 "tdmpc", "diffusion", "act",
              "vqbet", "pi0", "pi05", "sac", "reward_classifier", "smolvla", "wall_x" 等。

    返回:
        对应名称的策略类。

    异常:
        NotImplementedError: 如果策略名称未被识别。
    """
    if name == "tdmpc":
