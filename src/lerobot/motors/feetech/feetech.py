...
# robot.read("Present_Position", "motor_1")
#     ↓
# 查找寄存器地址（Control Table）
#     ↓
# scservo_sdk 构建数据包 [FF FF ID LEN 02 ADDR ...]
#     ↓
# 串口发送 → 电机响应
#     ↓
# decode_sign() 处理符号位
#     ↓
# normalize() 归一化到 [-1, 1] 或角度值
...
class FeetechMotorsBus(SerialMotorsBus):
    ...
    def __init__(
        ...
        #  Protocol 0（SCS协议）和 Protocol 1 两种
    ...
    def read_calibration(self) -> dict[str, MotorCalibration]:
        offsets, mins, maxes = {}, {}, {}
        for motor in self.motors:
            # 最小位置限制,最大位置限制,零点偏移（仅 Protocol 0）
    ...
    # 符号位编解码,Feetech 用最高位作为符号位，而非标准补码，所以需要特殊处理 将负数编码为符号位格式
    ...
    # Protocol 1 不支持 sync_read 和 broadcast_ping,需要逐个轮询电机，效率较低
