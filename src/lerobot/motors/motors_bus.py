"""
电机总线模块 (Motors Bus Module)
================================

本模块提供了与各种伺服电机通信的基础设施。

主要功能：
- 定义电机总线抽象接口 (MotorsBusBase)
- 电机标定数据结构 (MotorCalibration)
- 电机配置数据结构 (Motor)
- 控制表地址管理工具函数

支持的电机类型：
- Dynamixel: ROBOTIS 的 Dynamixel 系列电机
- Feetech: Feetech 系列电机
- Damiao: 达妙电机
- Robstride: Robstride 电机
...
class MotorsBusBase(abc.ABC):
    ...
    主要接口方法:
        ...
        - disconnect(): 断开连接
        - read()/write(): 单电机读写
        - sync_read()/sync_write(): 多电机同步读写
        - enable_torque()/disable_torque(): 力矩控制
        - read_calibration()/write_calibration(): 标定数据管理
    
    属性:
        port (str): 电机总线端口（如 "/dev/ttyUSB0"）
        motors (dict): 电机名称到 Motor 对象的映射
        calibration (dict): 电机名称到 MotorCalibration 的映射
    """
...
class SerialMotorsBus(MotorsBusBase):
    """
        串口电机总线:允许通过串口高效地读写电机数据
        菊花链连接:支持多个电机通过串口菊花链(daisy-chain)方式连接
        目前有两种实现:
        DynamixelMotorsBus - Dynamixel 电机（XL430, XL320, MX系列)
        FeetechMotorsBus - Feetech 电机(飞特 STS3215, SCS系列)
        lerobot-find-port.py
        # 会列出所有可用端口,帮你识别正确的端口

        例如，使用 Feetech 电机的总线配置如下：
        bus = FeetechMotorsBus(
            port="/dev/tty.usbmodem575E0031751",  # 串口地址
            motors={"my_motor": (1, "sts3215")},  # 电机配置: {名称: (ID, 型号)}
        )
        # DynamixelMotorsBus
